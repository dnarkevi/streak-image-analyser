...
def residual(param):
    ...
    return (func(param, times)[tr_1:tr_2] - decay[tr_1:tr_2]) / decay[tr_1:tr_2]**0.5
...
result_column = [
    ...
    [
        sg.Column([[sg.T("τ1 ="), sg.T(s = (7, None), key = '-OUT_T1-')], 
                   [sg.T("τ2 ="), sg.T(s = (7, None), key = '-OUT_T2-')], 
                   [sg.T("τ3 ="), sg.T(s = (7, None), key = '-OUT_T3-')]]),
    ...
    [
        sg.Column([[sg.T("τA ="), sg.T(s = (7, None), key = '-OUT_TA-')],
                   [sg.T("τI ="), sg.T(s = (7, None), key = '-OUT_TI-')]])
    ]    
]
#%%2ndColumn
second_column = [
...
try:
    while True:
        ...
        if event == '-UPDATE-':
            ...
            try: 
                ...
                sg.popup("Not enough memory. Increase number of frames to sum.", title = "Error")
                event = '-NONE-'
    #%%shift
        if event == '-UPDATE-':
    ...
    #%%tofit        
        ...
        if event == '-TO_FIT-': 
            ...
            times = ylin.astype(np.float64) #Also makes a copy.
            
            fig5.clp()
            fig5.ax1.set(ylim = (0.9*min(decay), 1.1*max(decay)), xlim = np.flip(ybd))
            fig5.ax2.set(xlabel = "Time, " + time_unit, xlim = np.flip(ybd))  
            fig5.ax1.scatter(times, decay, label = dec_names[dec_idx], s = 1, color = 'blue')
    ...
    #%%fit  
        ...
        if event == '-FIT-':
            ...
            fig5.ax1.plot(times, gauss(ylin, pout[1, 0], pout[1, 1]), color = 'orange') 
            parchk= np.concatenate((pout[1], pout[2:, :2].flatten()))
            fig5.ax1.plot(times[tr_1:tr_2], func(parchk, times)[tr_1:tr_2], color = 'red')
            fig5.ax1.autoscale()
            fig5.ax1.set(xlim = np.flip(ybd), ylim = (0.9*min(decay), 1.1*max(decay)))
            #Residuals.
            fig5.ax2.lines.clear()
            fig5.ax2.plot(times[tr_1:tr_2], res.fun, color = 'green') 
            fig5.ax2.autoscale()
            fig5.ax2.relim()
